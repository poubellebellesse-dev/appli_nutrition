...
>
> **Relevé du 2026-08-07, fin de journée** — `node -e` sur `app/public/catalog/catalog.db`, après le
> lot « 8 plats végétaliens sans gluten » :
>
> | Quantité | Valeur | Comment la remesurer |
> |---|---|---|
> | Recettes | **305** | `SELECT COUNT(*) FROM recipe` |
> | Aliments | **451** | `SELECT COUNT(*) FROM food` |
> | Étapes de recette | **1 415** | `SELECT COUNT(*) FROM recipe_step` |
> | Photos de recette | **0 / 305** | `SELECT COUNT(*) FROM recipe WHERE image_path IS NOT NULL AND image_path <> ''` |
> | `piquant` renseigné | **305 / 305** (293 à `0`, 12 à `1`) | `SELECT piquant, COUNT(*) FROM recipe GROUP BY 1` |
> | Créneaux | dîner **197** · déjeuner **172** · petit-déjeuner **43** · goûter **40** | agréger `recipe.types_repas` (JSON) |
> | Envergure | quotidien **233** · convivial **62** · fête **10** | `SELECT envergure, COUNT(*) FROM recipe GROUP BY 1` |
