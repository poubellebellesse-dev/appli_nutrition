...
| ~~43~~ | **Lexique de gestes — 4 fiches pour 93 étapes** | **COMPLÉTÉ le 2026-07-28.** Constat de l'audit du 2026-07-27, le dernier de ses points encore ouvert côté contenu. **4 → 43 gestes**, et surtout **155 → 615 étapes annotées** sur 1 097 (56 %). **Méthode** : les gestes ont été EXTRAITS des 1 097 étapes réelles, pas repris d'un glossaire culinaire. Six candidats (`zester`, `flamber`, `émulsionner`, `peler à vif`, `dégermer`, `torréfier`) ont été ÉCARTÉS parce qu'aucune recette ne les emploie — une fiche que personne ne référence est du contenu mort à maintenir. ⚠️ **Écrire les fiches ne suffisait pas** : 942 étapes portaient `lexicon_ids: []`. Une fiche non référencée est invisible dans l'appli. Les deux travaux — écrire ET câbler — ont été faits ensemble, 225 recettes recâblées. `tests/lexique-coherence.test.ts` verrouille les deux pourritures silencieuses possibles : une étape qui référence un code sans fiche (l'utilisateur clique et n'a rien) et une fiche jamais référencée. Aucune des deux ne ferait échouer le build |
