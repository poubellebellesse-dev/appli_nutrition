...
| 34 | **Les repas principaux du catalogue sont mal étiquetés** | **OUVERTE. ⚠️ DIAGNOSTIC CORRIGÉ le 2026-07-28** — la première version de cette décision disait « le catalogue est trop léger ». **C'est FAUX**, la mesure l'a démenti : la meilleure journée possible sur 3 repas atteint **2 127 kcal** (488 + 819 + 819). Le contenu suffit. **Cause réelle** : 61 des 183 recettes portant `dejeuner` ou `diner` apportent moins de 300 kcal, parce que des ENTRÉES, ACCOMPAGNEMENTS et DESSERTS sont étiquetés repas principaux — « Carottes Vichy » 147 kcal, « Œufs mimosa » 176, « Blancs en neige sucrés » 126, « Soupe de carottes à l'ail » **103**. En les retirant des créneaux principaux il reste 122 plats de médiane 432 kcal. **Le correctif moteur a été tenté et mesuré INSUFFISANT** : la cible nutritionnelle restante (§7.1) est codée, elle ne fait passer le pire jour que de 1 061 à 1 125 kcal. Arithmétique : `scoreNutri` moyenne sur 9 nutriments et `nutri` pèse 0,25, donc l'énergie vaut **2,8 %** de la note — aucune cible ne renversera un classement arbitré par la saison et les préférences. **Pistes** : **(a)** porter `CourseKind` sur `Recipe` (entrée/plat/accompagnement/dessert) et n'autoriser que les `plat` en créneau principal — traite la cause, mais demande d'annoter 212 recettes ; **(b)** re-étiqueter à la main les ~61 `types_repas` fautifs, moins structurant mais immédiat ; **(c)** relever le poids de `nutri`, qui déplacerait TOUT le classement pour un problème d'étiquetage. ⚠️ NE PAS affaiblir `assertCalorieFloor`. Reco : **(b) puis (a)** — corriger les données d'abord, structurer ensuite |
