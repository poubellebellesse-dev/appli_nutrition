...
**Prochaine étape : `suggestAlternatives`** (décision 26, §6) — le blocage qui le reportait est levé,
le catalogue a de quoi faire travailler le mécanisme « plat frère ».
...
| Code — `suggestAlternatives` (spec révisée : variante vs alternative) | `app/src/engine/selection/alternatives.ts` (à créer), `app/src/engine/api/index.ts` | ⬜ **DÉBLOQUÉ** — le report tenait à un catalogue de 10 recettes, il en compte 212. Prochaine étape |
