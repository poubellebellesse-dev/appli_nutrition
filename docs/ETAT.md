...
- ✅ **La quantité est DANS la phrase de l'étape, plus seulement en badge dessous — 2026-08-08.**
  ...
  fait donc au rendu (`app/src/ui/texte-etape.ts`, module pur, 41 tests), servi aux deux écrans.
  **1 828 liens posés sur 2 786, soit 1 086 gestes sur 1 397 (77,7 %).**
  ...
  **des** 50 g de beurre », « **chaque** banane » → « **les** 4 bananes », « la moitié **de l'**
  ...
  ⚠️ **Ce qui reste hors de portée d'une règle** : 958 liens non injectés, dont **364 libellés sans
  nombre** (« au goût ») — irrécupérables — et **77 étapes** où l'aliment n'est pas nommé, seul vrai
  levier restant, et qui demande une réécriture à la main du YAML.
...
- ⚠️ **LE LOT « SAUCES » S'ARRÊTE À LA CONSULTATION — RIEN N'EST ENCORE MÉMORISÉ NI ACHETÉ.** Ce qui
  existe au 2026-08-08 : le catalogue (3 sauces, 14 attachements), `engine/domain/sauces.ts`,
  `Engine.suggestSauces` (exclusions + garde-fou allergènes), et la section dépliante de la fiche
  recette. **Ce qui n'existe pas, et qu'il ne faut pas croire acquis** : ① **choisir une sauce ne
  laisse aucune trace** — aucune écriture en base, donc ses ingrédients **n'entrent pas dans la liste
  de courses** ; ② **aucune catégorie « sauces » dans les courses** ni dans l'écran Recettes, où les
  3 sauces ne sont atteignables que par la fiche d'un plat ; ③ **on ne peut pas cuisiner une sauce en
  même temps que son plat** — `user_cuisine_session` porte un `CHECK (id = 1)` et ne tient qu'UNE
  recette à la fois ; c'est le vrai coût du mode cuisine à deux recettes, et il n'a pas été payé ;
  ④ **l'éditeur ne permet pas d'attacher une sauce à une recette perso** (`user-recipe.ts` écrit
  `sauceIds: []` et `porteDejaUneSauce: null`, ce dernier faisant que la dérivation joue quand même).
  ⚠️ **Le point ③ est un changement de schéma `user.db`, pas un écran** : le chiffrer comme du
  travail d'interface est l'erreur à ne pas refaire.
