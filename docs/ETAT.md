...
**Suite exécutée le 2026-08-05** : `npm test` → **1 492 passed (84 fichiers)** en 38,8 s ·
...
(33 positions). **La sortie réelle fait foi, pas cette ligne.**
...
**Le catalogue est réel** : **450 aliments** aux valeurs CIQUAL 2025 de l'ANSES (plus aucun `PROV-`),
...
| **Contenu — état COURANT du catalogue** | `catalog/sources/foods.yaml`, `catalog/recipes/`, `catalog/lexicon/` | ✅ **450 aliments · 241 recettes · 62 gestes** (relevé du 2026-08-05 — 199 le 2026-07-29 : le catalogue a plus que doublé depuis, chantier de la session concurrente). Les deux cibles de la décision 4 (~200 aliments, 200-300 recettes) sont dépassées. Ⓐ Les lignes ci-dessus datent chacune de la clôture de son lot ; **celle-ci fait foi** |
...
- ✅ **Le lien étape → ingrédient existe — lot L2, 2026-08-07, ET IL N'A DEMANDÉ AUCUNE ANNOTATION.**
  ...
  dérive le lien du texte au build : **93,7 % des gestes** trouvent au moins un ingrédient, 1,9 %
  sont ambigus. Cinq mécanismes, dont trois hors plan — mot de tête sur les DEUX premiers mots (le
...
>
> **Réellement ouvertes au 2026-08-07 (fin de journée) : 2, 3, 5, 6, 11, 52, 60, 61, 62.** Parmi
> elles, quatre ne demandent pas un arbitrage mais un élément extérieur — 2 et 5 attendent une
> maquette, 3 un libellé, 6 un hébergeur, 11 un jeton de l'utilisateur. **Celles qui demandent une
> décision sont 52, 60, 61 et 62.** La **35 est sortie de cette liste** le 2026-08-07 : tranchée,
> codée, mesurée. La **62 y est entrée** le même jour — les 23 recettes sans facette `cuisine`.
>
...
>
> **Relevé du 2026-08-07, 11 h** — `node -e` sur `app/public/catalog/catalog.db` :
>
> | Quantité | Valeur | Comment la remesurer |
> |---|---|---|
> | Recettes | **297** | `SELECT COUNT(*) FROM recipe` |
> | Aliments | **450** | `SELECT COUNT(*) FROM food` |
> | Étapes de recette | **1 368** | `SELECT COUNT(*) FROM recipe_step` |
> | Photos de recette | **0 / 297** | `SELECT COUNT(*) FROM recipe WHERE image_path IS NOT NULL AND image_path <> ''` |
> | `piquant` renseigné | **297 / 297** (286 à `0`, 11 à `1`) | `SELECT piquant, COUNT(*) FROM recipe GROUP BY 1` |
> | Créneaux | dîner **184** · déjeuner **159** · petit-déjeuner **43** · goûter **40** | agréger `recipe.types_repas` (JSON) |
> | Envergure | quotidien **226** · convivial **56** · fête **10** | `SELECT envergure, COUNT(*) FROM recipe GROUP BY 1` |
> | Lexique · tips · fiches | **62** · **73** · **8** | `lexicon_entry`, `tip`, `evidence_sheet` |
> | Fichiers de test d'écran | **21** | `ls app/src/ui/**/*.test.tsx` |
>
...
- ✅ **Les écrans sont testés** (commits `568144b`, `bdcd0d3`) — **18 fichiers `*.test.tsx`** au
  2026-08-07 (relevé en tête de §8), contre 9 à l'ouverture de cette ligne. La dette « zéro test
  d'interface », n°1 de la fiche de reprise jusqu'au 2026-07-31, est close.
...
- ⚠️ **Les compteurs de pastilles coûtent une requête par facette.** Chaque facette est comptée
  ...
  retirer ramènerait 19 recettes. Deux requêtes de plus par changement de filtre, sur le catalogue courant (relevé en tête de §8) :
...
- ✅ ~~**Aucun test d'interface.**~~ **PÉRIMÉE, close le 2026-08-07 en la relisant.** Elle disait
  ...
  valider » — `jsdom` est en place depuis le 2026-07-31 et **18 fichiers `*.test.tsx`** montent de
  vrais écrans (`@vitest-environment jsdom`). Elle contredisait la ligne « Les écrans sont testés »
...
- **Zéro photo — 0 recette sur 292 au 2026-08-07** (relevé et requête en tête de §8). Le critère de sortie P6 (« bundle < 15 Mo », budget 40 Ko/image)
...
- ✅ **`Recipe.piquant` est renseigné sur les 297 recettes** (2026-08-07, décision 35) — 286 à `0`,
  ...
  ⛔ **`Food.piquant` N'EST PAS ANNOTÉ** — 450 aliments, tous `null`. `foods.yaml` était en cours
  d'édition par une autre piste le jour de la décision 35, et deux sessions dans le même fichier
  auraient produit un conflit. Verrouillé par test pour que `null` ne devienne pas `0` par défaut.
