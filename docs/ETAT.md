...
- **Gestes de cuisine** : boucle WebP 3 s pour les gestes simples ; **3 clips MP4 de 3 s**
  ...
  **Les 98 posters restent, 3,45 Mo.** La vignette de la ligne repliée réutilise le poster du
  ...
  `G:\Claude\Dessinateur\gestes` n'existe que sur une machine. C'est exactement la famille d'échec
...
⛔ **UN « FINI QUAND » QUI NE FAIT VARIER QU'UNE DIMENSION LAISSE PASSER LES CROISEMENTS — payé sur
ce lot, à retenir pour les suivants.** Les sept clauses faisaient varier la porte (nom, famille,
groupe) et l'état de la case ; **aucune ne faisait varier le régime déclaré**. Un retrait posé
avant qu'un régime plus strict ne rattrape l'aliment devenait indéfaisable — écrit en base, affiché
en texte mort. Les 12 tests scellés étaient verts pendant que la clause 5 était fausse. ⚠️ **Le
défaut n'était pas dans les tests, il était dans les clauses** : un test scellé ne peut pas
inventer une dimension que le « Fini quand » ne nomme pas. ▶ **À l'écriture du prochain brief :
lister les réglages persistants que l'écran lit (régime, allergènes, exclusions de groupe) et dire
explicitement lesquels la clause fait varier.** Trouvé à la relecture par deux agents
indépendamment, jamais à l'exécution.

⚠️ **UN FICHIER DE TEST PARAMÉTRÉ PAR LES DONNÉES CHANGE DE COMPTE SANS APPARAÎTRE DANS AUCUN DIFF
DE TEST.** `ui/parcours.test.tsx` est bâti sur `it.each` alimenté par la table `PARCOURS` : le lot
`retour-1b`, en ajoutant un parcours, lui a fait rendre **+90 tests** sans toucher une ligne de ce
fichier. Un écart de compte attribué « par diff des fichiers de test » conclut alors à un mystère.
▶ Mesuré, pas déduit : `git show <ancien>:app/src/ui/parcours.ts` puis relance du seul fichier →
144 contre 234. ⚠️ **Ce n'est PAS le symptôme documenté ailleurs** (un compte qui BAISSE sans rouge).

⚠️ **La passe à l'œil sur téléphone n'a pas été faite**, et ce lot lui ajoute deux écrans : le champ
de recherche (clavier qui recouvre les résultats) et une famille de 74 lignes à faire défiler.
▶ Elle rejoint celle déjà due par `retour-1` et `retour-1b` — une seule passe, pas trois.
...
~~⚠️ **`parcoursDeLEcran` (`ui/parcours.ts`) N'A AUCUN APPELANT**~~ — ✅ **FERMÉE le 2026-08-22
(lot `retour-2`) : la fonction a été RETIRÉE.** Elle n'avait aucun appelant depuis sa naissance,
**et elle était devenue fausse** : elle cherchait le parcours par son `ecran`, or depuis `retour-1b`
deux parcours partagent la même valeur (`null`). La garder « au cas où » revenait à laisser une API
qui rendrait le mauvais parcours au premier qui s'en servirait. Une note à sa place l'explique.

~~⛔ **LA GARDE NE FILTRE QUE LES OUTILS D'ÉDITION.**~~ — ✅ **FERMÉE le 2026-08-22 (lot
`retour-2`).** `.claude/hooks/garde.mjs` inspecte désormais aussi `Bash`, et refuse **deux formes,
pas plus** : une redirection dont la CIBLE tombe dans une zone protégée, et un verbe qui écrit
(`tee cp mv rm sed -i perl -i git checkout…`) dans le même segment qu'un chemin protégé. ⚠️ **On
ne parse pas le shell, et c'est délibéré** : un faux refus coûte plus cher qu'un trou résiduel,
d'autant que la parade d'un refus est d'utiliser `Edit`, que la garde sait lire — donc elle ne
bloque aucun travail légitime. ⛔ **CE QUI PASSE ENCORE, ET QU'IL FAUT SAVOIR** : un script tiers
(`node mon-script.mjs`) qui écrit dans une zone protégée, un `python -c`, une variable qui masque
le chemin. La garde est un garde-fou d'étourderie, pas un bac à sable. ▶ 12 formes ont été jouées
contre elle, y compris les cas qui doivent PASSER (`grep -rn tests/scelles/ > /tmp/out`,
`npx vite build`) — la sortie est dans le bilan du lot.
...
- ⚠️ **TROIS CASTS SONT LE SEUL ENDROIT DU DÉPÔT OÙ LA FORME INTERDITE EXISTE ENCORE, ET C'EST
  ...
  **polarité de repli** : en cas d'ignorance, rendre `corps` et `omnivore` plutôt que `production`
  et `vegetarien` — l'erreur qui retire un aliment de trop est réparable par l'utilisateur, celle
  qui en laisse passer un ne se voit pas. **Le cas reste atteignable à l'exécution** (recettes
  perso, contre un `user.db` sans clé étrangère vers le catalogue), donc le garde-fou garde son
  sens. ⛔ **La dette est que rien ne les distingue mécaniquement d'un cast de confort.** Chacun
  porte son commentaire ; un `grep` de `as AnimalSource` les retrouve tous les trois. Le jour où un
  quatrième apparaît sans commentaire, c'est un contournement, pas une mesure.
- ⚠️ **UN GARDE-FOU D'EXÉCUTION A ÉTÉ SUPPRIMÉ** dans `app/src/data/user-recipe.test.ts` : sa
  fabrique levait une exception quand une origine arrivait sans provenance. Le compilateur fait
  désormais le même travail plus tôt. **Ne pas le remettre** — il porterait sur un état devenu
  inatteignable par le type ; le commentaire laissé sur place le dit.
- ⚠️ **CE QUE LA 64 SIGNALAIT RESTE VRAI ET N'EST PAS DE CE LOT** : seule la branche
  mammifère/volaille consulte la provenance. Pour les poissons, fruits de mer et le miel, **aucun
  code ni test ne la relit** — les deux valeurs y sont légitimes, donc une erreur y reste
  structurellement indétectable. La forme garantit que la provenance est PRÉSENTE, jamais qu'elle
  est JUSTE.
- ⚠️ **LE CATALOGUE NE PEUT TOUJOURS PAS PROUVER LA CASCADE**, et le lot ne l'a pas changé : les
  **38** aliments dont l'origine vient de la dérivation résolvent tous `mammifere`/`production`, et
  **toutes les chaînes ont la longueur 1**. Une cascade qui rendrait cette paire en dur, ou qui ne
...
### Ce que le lot 65b laisse derrière lui (2026-08-18)

- ⛔ **LE SCEAU N'INTERDIT PAS QUE L'INTERRUPTEUR SOIT OUBLIÉ À LA FERMETURE — trou nommé par une
  relecture indépendante, inscrit en dette SUR DÉCISION DE L'AUTEUR plutôt que rouvert.** Aucun des
  20 tests scellés ne lit `user_equipment_filter` en SQL brut, et aucun ne rouvre la même base
  physique pour ne vérifier QUE le drapeau. Une implémentation qui garderait l'état dans une
  `WeakMap` indexée par l'objet `UserDb` passerait les vingt, et l'utilisateur retrouverait son
...
- ⚠️ **DÉCOCHER UN USTENSILE FILTRE ALLUMÉ NE REPASSE PAR AUCUN GARDE-FOU**, et c'est un choix, pas
  ...
  d'un coup sans qu'aucun compte n'ait été montré avant** — l'allumage à vide. ⛔ Le seuil est donc
  « zéro ustensile coché », pas « perte importante » : cocher un seul ustensile mal choisi peut faire
  tomber 264 recettes sans un mot. **Si ça se révèle mauvais à l'usage, c'est une décision à
  rouvrir, pas un défaut à corriger en silence.**
- ⚠️ **LA PREUVE PAR MUTATION VIT HORS DÉPÔT**, comme celle du lot 66c avant elle. Les 4 / 6 / 2
  rouges du tableau de `CONCEPTION_RESERVATION_MATERIEL.md` ne sont **rejouables que sur la machine
  qui les a produits**. ▶ Même dette que `atelier/mesure-occupation-four.mjs` et que les scripts du
  66c : **à trancher ensemble, pas séparément.**
- ⚠️ **`readOwnedEquipmentIds` REND TOUJOURS `null` SUR UNE TABLE VIDE**, et son en-tête décrit
  encore un futur qui est arrivé (« le jour où des Paramètres permettront de décocher, il faudra un
...
- ✅ **DETTE ANNULÉE LE JOUR DE SON OUVERTURE, LE 2026-08-14 — « les médias sont servis à une adresse
  devinable ». CE N'EST PAS UNE DETTE, ET LA GARDER EN AURAIT FAIT UNE.** Elle avait été inscrite en
  contrepartie de la décision 69, sur une rédaction erronée de celle-ci. **Trois vérifications l'ont
  démentie, et aucune ligne de code n'a été écrite entre-temps.** ① **`ARCHITECTURE.md` §7.1 impose
  l'adresse** — « médias hors du `.db`, fichiers référencés par chemin, jamais des blobs » — parce
  que c'est ce qui permet au service worker de ne re-télécharger que la photo modifiée. Une
  interdiction identique figure en §3. ② **Le remède envisagé aurait aggravé le mal** : 116 photos
  dans `catalog.db` s'extraient **par une seule requête SQL**, contre 116 noms de fichiers à deviner
  aujourd'hui. ③ **Et il coûtait cher pour ça** : `catalog.db` passerait de 1,7 à ~6,1 Mo chargés
  **en entier en mémoire au démarrage** (`ui/catalog-source.ts`), pour un utilisateur qui n'ouvrira
  peut-être aucune photo — plus la gestion à la main des `blob:` URL sur quatre écrans.
  ⛔ **CE QU'IL FAUT RETENIR, ET C'EST LA VRAIE LEÇON** : la contrepartie utile de la 69 n'a jamais
  été « pas d'adresse » mais « **pas de bouton** » — pas d'export, pas de partage, pas de galerie.
  Celle-là est tenue, gratuite, et se vérifie à l'œil. ⚠️ **Ne pas rouvrir cette dette sur une
  inquiétude** : elle a déjà été ouverte une fois par excès de zèle, et trois documents du dépôt
  disaient déjà non avant qu'elle soit écrite.
