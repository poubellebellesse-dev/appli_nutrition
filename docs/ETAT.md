...
| ~~31~~ | **`variety`/`habit` utilisaient encore `recipeMainIngredient`** | **Tranchée PAR MESURE et CODÉE (2026-07-27)** — règle de récence = **chevauchement de signature ≥ 0,45** (`VARIETY_RECENCY_OVERLAP_THRESHOLD`), au lieu de « même ingrédient le plus lourd ». Cinq règles comparées sur des paires jugées pour CETTE question, distincte de la similarité : l'ancienne déclenchait à tort sur 6 cas jugés sur 6 et touchait 326 paires ; la retenue en déclenche 0 et touche 86. Un repli par `Food.groupe` a été testé et écarté (4/6 faux, 735 paires) : « viandes » mélange bœuf, poulet, porc et agneau. **COMPLÉTÉE le 2026-07-27 par la SOUS-FAMILLE** : `Food.sousFamille` (facultatif, 25 aliments sur 193, 12 familles) replie `poulet_blanc`/`poulet_cuisse` sur `poulet` — d'un cran plus fin que `Food.groupe`, qui reste écarté. La récence lit désormais un **second index**, `recipeFamilySignature` ; la similarité garde `recipeSignature`, la diversification devant encore distinguer les morceaux. À seuil ÉGAL (0,45), la normalisation mesure pareil (0/6 faux, 1/7 ratés) et rattrape **16 paires légitimes** : lentilles vertes × lentilles corail (38 → 90 %), gigot × navarin d'agneau (14 → 65 %), huit paires de poulet dont curry × teriyaki (0 → 64 %). **ACHEVÉE le 2026-07-27 (§6.6 quinquies)** — 0 faux ET 0 raté, par deux ajouts mesurés. (a) **Second déclencheur** : une même sous-famille DÉCLARÉE pesant ≥ 40 % des deux côtés suffit, ce qui rattrape « poulet rôti aux carottes » (43 %) × « poulet au citron et aux olives » (71 %), que le seuil global à 39 % manquait. La restriction aux familles déclarées est essentielle — les clés d'une signature repliée mélangent familles et `foodId` bruts, et sans filtre partager `oeuf` rapprocherait une mousse au chocolat d'une omelette (3 faux sur 6). (b) **Filtre de créneau** : une entrée d'historique dont le `creneau` n'est pas dans les `typesRepas` du candidat est ignorée pour le rapprochement par composition — un clafoutis mangé au goûter ne pénalise plus un gratin de pâtes proposé au dîner. ⚠️ Ce n'est PAS « même créneau que la demande » : poulet au déjeuner puis poulet au dîner reste répétitif, et la correspondance par `recipeId` exact n'est jamais filtrée. Le modèle « ingrédient principal + secondaires à poids fixes » a été testé et ÉCARTÉ (2 à 3 ratés au lieu d'1) : les poids de rang détruisent l'écart réel entre 54 % et 43 % de protéine. **ANCIEN TEXTE :** Même index, même défaut : mesuré, 194 paires sur 290 (67 %) partagent un « ingrédient principal » tout en ayant une composition très différente — une mousse au chocolat rend « récentes » des galettes de sarrasin. Mais la correction n'est PAS celle de la similarité : « ai-je mangé ça récemment » est une autre question que « ces plats se ressemblent-ils ». Piste : chevauchement de signatures au-delà d'un seuil, le seuil restant à mesurer. **Ne pas recopier la signature ici sans rejouer une mesure dédiée** |
