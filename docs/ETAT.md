...
- ⚠️ **Un prérequis de la v1 reste à satisfaire, et ce n'est pas du câblage** : le lien
  ...
  alias). Spec : `ARCHITECTURE.md` §5bis. **Ordre des lots et méthode de montée :
...
- ✅ **L'écran du mode cuisine est codé — lot L1, 2026-08-05.** Écran allumé (`ecran-allume.ts`),
  ...
  d'entrée de visite guidée** — à trancher, pas à oublier. Restent **L2** (prérequis A) puis **L3**.
...
>
> **Réellement ouvertes au 2026-08-05 (fin de journée) : 2, 3, 5, 6, 11, 16, 35, 52** (+ 33, partiellement close). Parmi elles, six
> ne demandent pas un arbitrage mais du travail ou un élément extérieur — 2 et 5 attendent une
> maquette, 3 un libellé, 6 un hébergeur, 11 un jeton de l'utilisateur, 16 un chantier dont le
> préalable est levé. **Les deux qui demandent une décision sont 35 et 52** — plus la 33, tranchée le 2026-08-05 mais dont l'AFFICHAGE reste à décider (§8, « rempli et jamais lu »).
>
...
| 16 | Table courses non alimentaire (10 rayons) | Conçue ; à coder **quand `buildShoppingList` existera** (P1c+), pas avant. ⚠️ **LA CONDITION EST LEVÉE DEPUIS LE 2026-07-28** — `buildShoppingList` existe, rend une liste rangée par rayon et sert l'écran Courses. Ce n'est donc plus une décision en attente d'un préalable mais un **chantier ouvert**, non planifié |
...
- ⛔ **`food_nutrient.code_confiance` est REMPLI ET JAMAIS LU** (2026-08-05, décision 33). 3 907
  cotes ANSES sont importées, stockées, chargées jusqu'au `Socle` — et aucun écran ne les affiche.
  C'est nommément le défaut que ce projet paie en boucle (`note_allergene`, `Recipe.service`,
  `ratio`/`contexte`), assumé ici le temps d'une décision, pas ignoré.
  ⚠️ **La raison n'était pas prévisible depuis la décision** : `detail-recette.tsx` n'affiche que
  l'ÉNERGIE, et l'énergie est à **434 D sur 449 par construction** — une mention de provenance
  constante sur 97 % du catalogue serait du bruit, pas de la traçabilité. **Aucun écran n'affiche
  de valeur nutritionnelle par ALIMENT**, qui est le seul niveau où la cote discrimine.
  **Trois issues, aucune tranchée** : (i) annoter l'énergie quand même ; (ii) créer un écran de
  détail par aliment ; (iii) laisser dormir jusqu'à ce qu'un écran affiche des valeurs par aliment.
  ⚠️ Tant que rien n'est branché, **ne pas conclure que la décision 33 est faite** : le pipeline
  l'est, la décision non.
