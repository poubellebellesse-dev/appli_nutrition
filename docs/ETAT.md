...
| 34 | **Les repas principaux du catalogue sont mal étiquetés** | **PARTIELLEMENT TRAITÉE (2026-07-28)** — `CourseKind` est désormais porté par `Recipe` (colonne `service`), et **les 212 recettes sont annotées** : 125 plats, 39 entrées, 27 desserts, 21 accompagnements, 0 fromage. ⚠️ **Le point clé, qui avait été mal compris au départ** : `service` et `types_repas` sont des axes **ORTHOGONAUX**, pas des alternatives. Une purée est un `accompagnement` servi au `dejeuner` ET au `diner` — annoter le rôle ne retire personne d'un créneau. Vouloir « sortir » les accompagnements des créneaux principaux les aurait fait DISPARAÎTRE de l'appli, `MealSlot` n'ayant aucune case pour eux. ⚠️ **Le plancher calorique n'est PAS réglé pour autant** : un accompagnement peut être servi seul (décision utilisateur), donc « Carottes Vichy » reste un dîner valide et une journée à 3 repas peut toujours passer sous 1 200 kcal. Ce qu'il reste à faire est une ÉTAPE DE RÉPARATION dans `planWeek` (quand une journée finit sous le plancher, re-choisir le créneau le plus faible sous contrainte d'énergie) — décidé nulle part encore. ⚠️ NE PAS affaiblir `assertCalorieFloor` |
...
| 37 | **Le catalogue n'a QU'UN SEUL petit-déjeuner végétalien** | **OUVERTE, mesurée le 2026-07-28 au banc de stress.** Répartition végétalienne : 23 déjeuners, 18 dîners, 3 goûters, **1 petit-déjeuner**. Conséquence directe : un plan végétalien de 14 jours × 3 créneaux ne peut PAS dépasser `1 + 14 + 14 = 29` créneaux remplis sur 42, et `planWeek` en place exactement 29 — il fait l'optimum, la limite est le contenu. Le régime végétarien n'a pas ce problème (16 petits-déjeuners). À traiter au prochain lot de contenu : ~10 petits-déjeuners végétaliens suffiraient |
