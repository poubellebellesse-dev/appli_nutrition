...
| [reference/CONCURRENCE_ET_ATTENTES.md](./reference/CONCURRENCE_ET_ATTENTES.md) | Ce que les utilisateurs demandent — et reprochent — aux applications comparables. Sourcé et daté, avec la qualité de chaque source : plusieurs des formules les plus citées viennent de blogs de concurrents. Relevé le 2026-08-04 · **§6 ajouté le 2026-08-16 : le paysage concurrentiel**. ⭐ Sa §6.3 change l'argumentaire — **ne plus vendre « sans IA » mais « déterministe, auditable, sûr sur les allergies »**, appuyé sur un cas daté d'IA ignorant une allergie déclarée. §6.1 : **la case « gratuit + local + hors-ligne sur mobile » est vide** (les libres sont auto-hébergés, donc hors public). §6.5 : signal Jow **non confirmé**, à revérifier à la main |
...
| Document | Sujet | État |
|---|---|---|
| [CONCEPTION_RETOURS_TEST.md](./CONCEPTION_RETOURS_TEST.md) | **Plan de montée** des retours du test sur téléphone du 2026-08-21 — huit lots `retour-1` à `retour-8`, nés de neuf arbitrages (décisions 71 à 78, plus la 81). ⛔ **Son fait fondateur : la donnée était juste et l'écran mentait** — le défaut n'était pas au moteur | ✅ `retour-1` (`3ce17d7`) et `retour-1b` (`42491ea`) livrés le 2026-08-21 ; `retour-2` à `retour-8` non ouverts. ⛔ **La passe à l'œil sur téléphone, NEUF cases, n'est pas faite** |
| [CONCEPTION_GESTES_ILLUSTRES.md](./CONCEPTION_GESTES_ILLUSTRES.md) | **Plan de montée** des gestes illustrés — le champ média du lexique, l'import des clips, l'affichage, le hors-ligne. ⛔ **Un geste s'illustre en CLIP, pas en photo** : une image fixe ne montre ni l'avant ni l'après, jamais le passage | Lots geste 1, 3 et 4 livrés le 2026-08-16 ; **le lot geste 2 est ARRÊTÉ à 3 gestes sur 51, par décision** — les binaires se gravent pour toujours dans l'historique git |
| [CONCEPTION_PHOTOS_RECETTES.md](./CONCEPTION_PHOTOS_RECETTES.md) | **Plan de montée** des photos de recettes — l'affichage, le cadre honoré à l'import, la photo sur la fiche détail | Lots photo 1 (`f3d4fa1`) et 3 (`77ca4dc`) livrés ; **129 sur 330**. ⛔ **Ce qui reste bloque sur la SOURCE, pas sur le code** — le bac est épuisé |
| [CONCEPTION_INVARIANT_ORIGINE_ANIMALE.md](./CONCEPTION_INVARIANT_ORIGINE_ANIMALE.md) | **Plan de montée** de l'invariant origine/provenance — une PAIRE indivisible ou rien, garantie par la FORME du type et non par une validation. ⛔ **Une paire de champs n'a pas deux cas de défaillance mais QUATRE** | ✅ 66 (`ad1ad47`), 66b (`17b7700`) et 66c (`e552ca1`) livrés ; les six cases connues sont closes |
| [COURRIERS_SOURCES.md](./COURRIERS_SOURCES.md) | Les courriers envoyés aux détenteurs de sources et de médias, et leurs réponses | Vivant — un courrier reste à envoyer (Pixabay) |
